...
var Presets = []PresetInfo{
	...
	{"walk", "걷기", "기본 동작", "side-view walking cycle facing right", 8, 12, true, "Side-view walk cycle that MUST alternate feet — this is critical. Treat the frames as TWO mirrored steps: the FIRST half is a LEFT-leg-forward step (left foot strikes forward → body passes over the planted foot → left leg pushes off behind), and the SECOND half is its exact MIRROR — a RIGHT-leg-forward step. The frame at the EXACT MIDDLE of the cycle must show the RIGHT leg forward and left leg back — the opposite of the first frame. NEVER step twice with the same leg, and never plant both feet forward. The arm OPPOSITE the forward leg swings forward (right arm with left leg, and vice versa). Gentle 1-2px body bob, evenly-timed distinct phases, last frame hands seamlessly into the first."},
	{"run", "달리기", "기본 동작", "fast side-view running cycle facing right", 8, 14, true, "Side-view run cycle that MUST alternate feet. TWO mirrored strides: first half = LEFT-leg-drive stride, second half = mirrored RIGHT-leg-drive stride; the exact MIDDLE frame shows the RIGHT leg forward (opposite of frame 1's left). Strong forward lean, big leg extension, an airborne (both feet off ground) frame between each contact, arms pumping with the opposite leg. Never two strides on the same leg. Evenly-timed distinct phases, seamless loop."},
	{"sprint", "전력 질주", "기본 동작", "all-out sprint, extreme lean and stride", 8, 16, true, "Side-view all-out sprint that MUST alternate feet. TWO mirrored strides (LEFT-drive then RIGHT-drive); the exact MIDDLE frame shows the RIGHT leg forward, opposite of the first. Extreme forward lean, maximal leg extension, both feet airborne at each stride's peak, arms pumping hard with the opposite leg. Never two strides on the same leg. Faster, larger strides than a run; evenly-timed distinct phases, seamless loop."},
